<aside class="bg-gray-800 min-h-screen min-w-64">
    <div class="flex flex-col justify-between h-full mx-6">
        ...
        <!-- Current Topic -->
        <div class="mt-5 mb-2 px-4 py-2 bg-gray-700 rounded-md text-sm font-medium">
        ...
        <nav class="mt-2 flex-grow">
            @foreach($topic->chapters as $chapter)
                <div class="flex flex-col">
                    <a href="{{ route('chapters.show', ['topic' => $topic->id, 'chapter' => $chapter->id]) }}" class="flex items-center text-gray-300 hover:bg-gray-700 hover:text-white px-3 py-4 rounded-md text-sm font-medium">
                        <!-- Add icon here -->
                        ...
                        <!-- Chapter title -->
                        <span style="color: #FFFFFF; font-size: 0.9rem;">{{ $chapter->title }}</span>
                    </a>
                </div>
            @endforeach
        </nav>
